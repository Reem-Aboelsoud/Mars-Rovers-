...
class MarsRover():
    ...
    def r_rotation(self):
        angle = directions[self.current_position[2]] - 90        #takes the angle associated with each compass direction and subtract 90 from it
        angle = angle % 360                                      # takes care of the boundary condition 
        self.current_position[2] =  directions_inv[str(angle)]   #after subtracting giving the result angle, this gives the direction associated with it
    ...
    def l_rotation(self):
        angle = directions[self.current_position[2]] + 90        #takes the angle associated with each compass direction and add 90 from it
        angle = angle % 360                                      #takes care of the boundary condition 
        self.current_position[2] =  directions_inv[str(angle)]   #after subtracting giving the result angle, this gives the direction associated with it
        return self.current_position[0], self.current_position[1], self.current_position[2]  
    ...
    def moving_forward(self):
        ...
        if directions[self.current_position[2]] ==90:     #if the current direction is N and the angle is 90, the with this function, 1 should be added to the y value 
            pos[1] = int(pos[1]) + 1
        elif directions[self.current_position[2]] ==270:  #if the current direction is S and the angle is 90, the with this function, 1 should be subtracted to the y value
            pos[1] = int(pos[1]) - 1
        elif directions[self.current_position[2]] ==180:  #if the current direction is W and the angle is 90, the with this function, 1 should be subtracted to the x value
            pos[0] = int(pos[0]) - 1
        elif directions[self.current_position[2]] ==0:    #if the current direction is E and the angle is 90, the with this function, 1 should be added to the x value
            pos[0] = int(pos[0])  + 1
        return pos
    ...
    def general_movement(self):
        ...
        if(int(x_pos) < 0 or int(y_pos) < 0 ): 
            raise Exception("The last position of the rover, should have only positive x and y values. Sorry, start again!")
    ...
    def checking_the_input(self): 
        ...
        if(len(self.current_position) != 3):            #Makes sure that the input of the current position is 3 values separated by a space
            raise Exception("The current Position of the rover should be 3 values (as :X_value Y_value cardinal compass direction), separated by a space. Try again!")
        ...
        if(x_pos.isdigit() == False or y_pos.isdigit() == False): #Makes sure that the current X_value and Y_value are integers
            raise Exception("X and Y values should be positive numbers only. Try again!. ")
            
        if(int(x_pos) > int(x_upp_right) or int(y_pos) > int(y_upp_right)): #Makes sure that the x and y values are not more than the x and y values of the upper right corner. 
            raise Exception("X and y values should be less than the upper right corner values. Try again!")
            
        if(dirn.isalpha() == False or dirn not in MarsRover.direction_letters): #Makes sure that the given direction is only E (for East) or W (for West) or S (for South) or N (for North)
            raise Exception("The given direction should be on of the cardinal compass direction (as a one letter: E or W oe S or N). Try again!")
    
        
        if(self.instructions.isalpha() == False or all(chars in ['L', 'R', 'M'] for chars in self.instructions) == False): #Makes sure that the given instructions are on of the following: R or M or L
